...
if not exist "%FRPC_EXE%" (
    echo 错误：未找到 frpc.exe，路径：%FRPC_EXE%   Error: frpc.exe not found, path:%FRPC_EXE%
    pause
    goto :eof
)
if not exist "%FRPC_CONFIG%" (
    echo 错误：未找到 frpc.toml，路径：%FRPC_CONFIG%    Error: frpc.toml not found, path:%FRPC_CONFIG%
    pause
    goto :eof
)
